...
/*   Updated: 2026/03/05 14:54:58 by adamjurczyk      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((current->type == FROM_FILE || current->type == REPLACE) && !current->next) // > at end
		return (exit_syntax_error(current->token));
	if ((current->type == FROM_FILE || current->type == REPLACE) && current->next && current->next->type == PIPE)
		return (exit_syntax_error(current->token));
	if ((current->type == FROM_FILE || current->type == REPLACE) && current->next && // > < or < >
		(current->next->type == FROM_FILE || current->next->type == REPLACE))
		return (exit_syntax_error(current->token));
	return (0);
}
